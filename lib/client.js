...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      /**
       * Pair the anchor's children to entry ids and reorder them to the
       * configured sequence. Each rendered list entry contributes exactly one
       * child of the anchor, in render order, so children can be paired to the
       * sorted registry ids — unless some entry rendered nothing (e.g. a
       * collapsed rail hiding a readout); in that ambiguous case the pass is
       * skipped rather than misassigning.
       */
      function reconcile() {
        ...
        // Drop mappings for ids that no longer exist.
        var stale = [];
        state.idByEl.forEach(function (value, key) {
          if (!valid[value]) stale.push(key);
        });
        for (var s = 0; s < stale.length; s += 1) state.idByEl.delete(stale[s]);

        // Split children into already-paired (known) and unpaired (unknown).
        var known = [];
        var unknown = [];
        for (var c = 0; c < children.length; c += 1) {
          var el = children[c];
          var id = state.idByEl.get(el);
          if (id && valid[id]) known.push([el, id]);
          else unknown.push(el);
        }

        var claimed = {};
        for (var k = 0; k < known.length; k += 1) claimed[known[k][1]] = true;
        var unclaimed = [];
        for (var u = 0; u < ids.length; u += 1) {
          if (!claimed[ids[u]]) unclaimed.push(ids[u]);
        }

        if (unknown.length !== unclaimed.length) return; // ambiguous — skip

        for (var p = 0; p < unknown.length; p += 1) {
          state.idByEl.set(unknown[p], unclaimed[p]);
        }
        ...
        var ordered = children.slice().sort(function (a, b) {
          var pa = pos[state.idByEl.get(a)];
          var pb = pos[state.idByEl.get(b)];
