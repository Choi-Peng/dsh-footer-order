...
//
// Inside settingsArea the configured order is pinned twice: by DOM
// re-ordering and by an inline `order` declaration with `!important` on every
// item. Flex containers lay items out by CSS `order` first, DOM order second,
// so without the inline pin a foreign stylesheet rule (e.g. dsh-restart's
// `#dsh-restart-btn{order:9999 !important}`) would keep an element visually
// last no matter where the plugin moves it in the DOM.
      ...
      function reconcile() {
        ...
        try {
          ...
          if (triggerRow && triggerRow.children) {
            ...
            // Clear any previously injected inline order values first.
            for (var tc = 0; tc < triggerRow.children.length; tc += 1) {
              triggerRow.children[tc].style.order = '';
            }
            if (sOrder && sOrder.length > 0) {
              // Build a key→element map for every direct child.
              var keyMap = {};
              var allChildren = Array.prototype.slice.call(triggerRow.children);
              for (var ti = 0; ti < allChildren.length; ti += 1) {
                var tChild = allChildren[ti];
                var isTrigger = tChild.tagName === 'BUTTON' && tChild.getAttribute('aria-haspopup') === 'dialog';
                ...
                var tKey = isTrigger ? 'settings' : isIndicator ? 'connection' : (tChild.id || tChild.getAttribute('data-slot') || ('item-' + (ti + 1)));
                keyMap[tKey] = tChild;
              }
              // Append nodes in the configured order, then append anything left over
              // at the end (preserving their relative DOM order).
              var seen = {};
              var ordered = [];
              for (var si = 0; si < sOrder.length; si += 1) {
                var sk = sOrder[si];
                if (keyMap[sk] && !seen[sk]) { seen[sk] = true; ordered.push(keyMap[sk]); }
              }
              for (var ri = 0; ri < allChildren.length; ri += 1) {
                var rc = allChildren[ri];
                // Find its key to check if already placed.
                var isTriggerR = rc.tagName === 'BUTTON' && rc.getAttribute('aria-haspopup') === 'dialog';
                var isIndicatorR = (rc.className && rc.className.indexOf('indicator') !== -1) || rc.hasAttribute('data-phase') || rc.getAttribute('role') === 'status';
                var rKey = isTriggerR ? 'settings' : isIndicatorR ? 'connection' : (rc.id || rc.getAttribute('data-slot') || ('item-' + (ri + 1)));
                if (!seen[rKey]) { seen[rKey] = true; ordered.push(rc); }
              }
              // Re-insert in desired order only if something changed.
              var needsMove = false;
              for (var oi = 0; oi < ordered.length; oi += 1) {
                if (triggerRow.children[oi] !== ordered[oi]) { needsMove = true; break; }
              }
              if (needsMove) {
                for (var mi = 0; mi < ordered.length; mi += 1) triggerRow.appendChild(ordered[mi]);
              }
              // DOM order alone does not control flex layout: flex items are
              // laid out by their CSS `order` first, document order second, so
              // a foreign stylesheet rule pinning `order` on one item would
              // keep it visually last wherever the plugin moves it in the DOM.
              // Pin the visual order with inline `order` + `!important` — an
              // inline style declaration outranks any stylesheet rule at equal
              // importance. The clear pass above (and the empty-settingsOrder
              // path) removes these again, restoring the foreign rule's effect.
              for (var pi = 0; pi < ordered.length; pi += 1) {
                ordered[pi].style.setProperty('order', String(pi), 'important');
              }
            }
          }
          ...
          try {
            ...
            if (triggerRow && triggerRow.children) {
              for (var i = 0; i < triggerRow.children.length; i += 1) {
                ...
                var isTrigger = el.tagName === 'BUTTON' && el.getAttribute('aria-haspopup') === 'dialog';
